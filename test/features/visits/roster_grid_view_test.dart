...
void main() {
  testWidgets('shows Unfilled row and day headers', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(body: RosterGridView(grid: grid, onTileTap: (_) {})),
      ),
    );
  ...
  testWidgets('name cell height matches multi-tile day cell row', (
    ...
    final nameRect = tester.getRect(
      find.byKey(const ValueKey('roster-name-jane')),
    );
  ...
  testWidgets('day header stays pinned while scrolling rows', (tester) async {
    ...
    final people = List.generate(
      30,
      (i) =>
          RosterPerson(contractorId: 'person-$i', displayName: 'Contractor $i'),
    );
    final grid = buildRosterGrid(
      rangeStart: monday,
      dayCount: 5,
