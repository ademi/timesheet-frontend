...
void main() {
  ...
  test('load shows repair banner when GET body_invalid flag set', () {
    ...
    c.applyLoadedPlan(_plan(body: const SupportPlanBody(), bodyInvalid: true));
  ...
  test(
    ...
    () async {
      ...
      verify(
        () => mock.patchSupportPlan('client-1', 'plan-1', any()),
      ).called(1);
    },
  );
    ...
    c.applyLoadedPlan(
      _plan(
        body: const SupportPlanBody(
          disabilityHealth: DisabilityHealthSection(primaryDisability: 'ASD'),
        ),
  ...
  test(
    'activate failure leaves prior status; success applies DTO status',
    () async {
      c.applyLoadedPlan(
        _plan(
          body: const SupportPlanBody(
  ...
  test(
    'save draft blocked when limitation Other selected without detail',
    () async {
  ...
  test(
    'activate blocked when communication Other selected without detail',
    () async {
      c.applyLoadedPlan(
        _plan(
          body: const SupportPlanBody(),
  ...
  test(
    'save draft includes Other detail keys in body_json when filled',
    () async {
      c.applyLoadedPlan(_plan(body: const SupportPlanBody(), id: 'plan-1'));
      c.functionalLimitations.add(SupportPlanKeys.limitationOther);
