...
class _AuthInterceptor extends Interceptor {
  ...
  Future<void> _refreshOrWait() async {
    ...
    if (refreshToken == null || refreshToken.isEmpty) {
      throw DioException(
        requestOptions: RequestOptions(path: '/auth/refresh', baseUrl: AppConstants.baseUrl),
...
/// Dio singleton: [plainDio] for login/refresh (no interceptors);
/// [dio] for authenticated calls with Bearer + 401 refresh retry.
class ApiClient {
  ...
  /// Unauthenticated client: `/auth/login`, `/auth/refresh`.
  final Dio plainDio;

  /// Authenticated client: attaches Bearer, refreshes on 401.
  final Dio dio;
}
