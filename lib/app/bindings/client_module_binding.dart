...
abstract final class ClientModuleBinding {
  ClientModuleBinding._();
  ...
  static void ensureDependencies() {
    ...
    if (!Get.isRegistered<ClientRemoteDataSource>()) {
      Get.put<ClientRemoteDataSource>(
        ClientRemoteDataSource(dio: Get.find<AttendanceApiClient>().dio),
        permanent: true,
      );
    }
    if (!Get.isRegistered<ClientRepository>()) {
      Get.put<ClientRepository>(
        ClientRepository(
          remote: Get.find<ClientRemoteDataSource>(),
