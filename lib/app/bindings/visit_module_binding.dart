...
abstract final class VisitModuleBinding {
  VisitModuleBinding._();
  ...
  static void ensureDependencies() {
    ...
    if (!Get.isRegistered<VisitRemoteDataSource>()) {
      Get.put<VisitRemoteDataSource>(
        VisitRemoteDataSource(authenticatedDio: Get.find<AttendanceApiClient>().dio),
        permanent: true,
      );
    }
    if (!Get.isRegistered<VisitRepository>()) {
      Get.put<VisitRepository>(
        VisitRepository(
          remote: Get.find<VisitRemoteDataSource>(),
